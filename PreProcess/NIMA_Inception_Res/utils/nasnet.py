...
def _separable_conv_block(ip, filters, kernel_size=(3, 3), strides=(1, 1), weight_decay=5e-5, id=None):
    ...
    with tf.name_scope('separable_conv_block_%s' % id):
        x = Activation('relu')(ip)
        x = SeparableConv2D(
            filters, kernel_size, strides=strides, name='separable_conv_1_%s' % id,
...
def _adjust_block(p, ip, filters, weight_decay=5e-5, id=None):
    ...
    with tf.name_scope('adjust_block'):
        if p is None:
            ...
            with tf.name_scope('adjust_reduction_block_%s' % id):
            ...
            with tf.name_scope('adjust_projection_block_%s' % id):
                p = Activation('relu')(p)
...
def _normal_A(ip, p, filters, weight_decay=5e-5, id=None):
    ...
    with tf.name_scope('normal_A_block_%s' % id):
...
def _reduction_A(ip, p, filters, weight_decay=5e-5, id=None):
    ...
    with tf.name_scope('reduction_A_block_%s' % id):
        p = _adjust_block(p, ip, filters, weight_decay, id)
...
def _add_auxiliary_head(x, classes, weight_decay):
    ...
    with tf.name_scope('auxiliary_branch'):
